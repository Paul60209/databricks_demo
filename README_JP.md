...
`chainlit_app.py` の `RunnableConfig` は各 run に `session_id` メタデータと `sony-bi-chatbot` タグを付与し、LangSmith ダッシュボードでのセッション別フィルタリングやコスト集計を可能にします。
